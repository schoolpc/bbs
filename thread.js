...
dbref.on('child_added', function (snapshot) {
    ...
    if (data.type === "resu" && data.ID === thID) {
        if ( username == nameField.val() ) {
            ...
            var messageElement = $("<il><p class='sender_name me'>" + numb + " " + username + "</p><br><div><div class='right_balloon me'>" + msg + "</div></div><p class='clear_balloon'></p></il>");
    ...
    if (data.type === "thread" && data.ID === thID) {
        var titleElement = $("<il><div class='card mt-2 mx-3 mb-0 p-3' style='display:inline-block;'><h4 class='m-0'>" + message + "</h4>" + username + " | " + time + "</div></il>");
