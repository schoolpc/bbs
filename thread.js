...
dbref.on('child_added', function (snapshot) {
    ...
    if (data.type === "thread" && data.ID === thID) {
        var titleElement = $("<il><div><div class='card mt-3 mx-3 mb-1 p-3' style='display:inline-block;'><h4 class='m-0'>" + message + "</h4>" + username + " | " + time + "</div></div><p class='clear_balloon'></p></il>");
...
dbref.on("child_added", (snapshot)=> {
    ...
    if(type === "resu"){
        var messageElement = $("<il><div class='card my-1 p-3' style='display:inline-block;'>" + name + " | <time class='time'>" + dat + "</time><p id='resu'>" + text + "</p><a class='name' href='thread.html?t="+ ID +"'>元スレ</a></div><p class='clear_balloon'></p></il>");
    }
    //HTMLに取得したデータを追加する
    newList.prepend(messageElement);
});